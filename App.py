...
connection = pymysql.connect(host='localhost',user='root',password='sql567',db='sra')
...
def run():
    ...
    if choice == 'Normal User':
        ...
        ## Admin Side
        st.success('Welcome to Ashish')
        # st.sidebar.subheader('**ID / Password Required!**')

        ad_user = st.text_input("Username")
        ad_password = st.text_input("Password", type='password')
        if st.button('Login'):
            if ad_user == 'code' and ad_password == 'A@123':
                st.success("Welcome Ashish Gaur")
                # Display Data
